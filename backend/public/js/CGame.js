function CGame(oData){
    ...
    this._init = function(){
       ...
       if (_bAdvancedMode){
        ...
        console.log('CGame PLAYER_1',PLAYER_1)
        _aPlayersStick[PLAYER_1]=new CStick(CANVAS_WIDTH/2,CANVAS_HEIGHT-130,BLUE_STICK, EDGEBOARD_X+2+100,CANVAS_WIDTH-(EDGEBOARD_X+2+100),[{x1: CANVAS_WIDTH/2+70, x2:CANVAS_WIDTH/2-70}],GOALKEEPER,PLAYER_SPEED_STICKS,PLAYER_1);
        _aPlayersStick[PLAYER_2] = new CStick(CANVAS_WIDTH/2,130,RED_STICK, EDGEBOARD_X+2+100,CANVAS_WIDTH-(EDGEBOARD_X+2+100),[{x2:CANVAS_WIDTH/2+70,x1:CANVAS_WIDTH/2-70}],GOALKEEPER,CPU_SPEED_STICKS,PLAYER_2);

        if(playerType == 'host'){
            //_aPlayersStick[PLAYER_2].setVisible(false);
        }else{
            //_aPlayersStick[PLAYER_1].setVisible(false);
        }
        ...
        if (s_b2Players===true&&!s_bMobile){
            if (s_bFirstMultiPlayer===true){
                ...
                s_bFirstPlay = false;
                s_oTutorial = new CPanelTutorial(true);
            }
        }
    ...
    this.sortObjectsByY = function(aObjects){
        ...
        this.remoteUpdateSpritePosition = function(xVal,yVal){
            for (var i=0;i<_aBall.length;i++){
                  _aBall[i].remoteUpdateSpritePosition(xVal,yVal);
            }
        }
        this.onMouseDownMsg = function(xVal,yVal){
            this.onMouseDown2('player',xVal,yVal);
        }
        this.onMouseDown = function(user,xVal,yVal){
            
            s_oGame.onMouseDown2();
        }
        this.onMouseDown2 = function(user,xVal,yVal){
            console.log('onMouseDown2');
            if(ballMoveInProgress){
                return;
            }
            
            ballMoveInProgress = true;
            ...
            console.log('1vLaunch',vLaunch);          
            vLaunch.normalize();
            console.log('2vLaunch',vLaunch);
            vLaunch.scalarProduct(BALL_START_VELOCITY);
            console.log('3vLaunch',vLaunch);
            if(user != 'player'){
                _aBall[0].vCurForce().setV(vLaunch);
            }else{
                _aBall[0].vCurForce().setVRemote(xVal,yVal);
            }
            console.log('xVal',vLaunch.getX())
            console.log('yVal',vLaunch.getY())
            if(user != 'player'){
                socket.emit("gamePlay",{move:'mousedown', gameData:{xVal:vLaunch.getX() , yVal:vLaunch.getY()},playerUserName:playerUserName, playerType:playerType,gameId:gameId,roomId:roomId});
            }
    ...
    this.updateRemote = function(data){
        if(playerType == 'host'){
            ...
            //_aPlayersStick[PLAYER_1].setVisible(false);
            _aPlayersStick[PLAYER_1].__updateStickPositionsRemote(data.gameData.iPlacement);
        }
